...
export function Sidebar({ role, collapsed = false }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "hidden shrink-0 overflow-hidden border-r border-slate-200 bg-white transition-[width,border-color] duration-200 md:block",
        collapsed ? "w-0 border-transparent" : "w-64"
      )}
      aria-hidden={collapsed}
    >
      <div className={cn("w-64 transition-opacity duration-150", collapsed && "pointer-events-none opacity-0")}>
